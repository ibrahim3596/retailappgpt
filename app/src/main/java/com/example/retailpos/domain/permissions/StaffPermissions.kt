package com.example.retailpos.domain.permissions

import com.example.retailpos.auth.UserRole

/**
 * Role-based authorization for sensitive billing and inventory actions.
 * Server/local enforcement lives with the repositories; this is the single
 * source of the rules themselves.
 */
enum class StaffRole { OWNER, MANAGER, CASHIER }

/** Maps the persisted UserRole to the authorization role. */
fun UserRole.toStaffRole(): StaffRole = when (this) {
    UserRole.OWNER -> StaffRole.OWNER
    UserRole.MANAGER -> StaffRole.MANAGER
    UserRole.CASHIER -> StaffRole.CASHIER
    else -> throw IllegalArgumentException("Unknown UserRole: $this")
}
...
object StaffPermissionRules {
    ...
    fun itemDiscountAuthorization(role: StaffRole): DiscountAuthorization = when (role) {
        StaffRole.OWNER -> DiscountAuthorization(true, 100.0)
        StaffRole.MANAGER -> DiscountAuthorization(true, 30.0)
        StaffRole.CASHIER -> DiscountAuthorization(false, 0.0, "Cashiers cannot apply item-level discounts.")
    }
    ...
    fun validateItemDiscount(role: StaffRole, lineSubtotal: Double, amount: Double): String? {
        if (!lineSubtotal.isFinite() || lineSubtotal < 0.0) return "Invalid item subtotal."
        if (!amount.isFinite() || amount < 0.0) return "Discount must be non-negative and finite."
        if (amount > lineSubtotal + 1e-9) return "Discount cannot exceed the item subtotal."
        val authorization = itemDiscountAuthorization(role)
        if (!authorization.allowed) return authorization.reason ?: "This staff role cannot apply item discounts."
        val percent = if (lineSubtotal == 0.0) 0.0 else amount * 100.0 / lineSubtotal
        if (percent > authorization.maxPercent + 1e-9) {
            return "${role.name.lowercase().replaceFirstChar { it.uppercase() }} item discount limit is ${authorization.maxPercent}%."
        }
        return null
    }
