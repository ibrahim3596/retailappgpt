...
/**
 * Safe correction signals from retailer review. These are suggestions for future ranking,
 * never automatic product identity changes.
 */
...
object ProductIdentificationFeedbackRules {
    fun toSignal(feedback: ProductIdentificationFeedback): ProductIdentificationFeedbackSignal {
        ...
        return ProductIdentificationFeedbackSignal(
            ...
            explanation = when {
                feedback.rejectedCatalog || feedback.correctedName || feedback.correctedBrand || feedback.correctedPack ->
                    "Retailer correction indicates lower trust in the original candidate."
                feedback.acceptedCatalog ->
                    "Retailer accepted the catalog candidate; this may be used as future ranking evidence."
                feedback.retainedCameraData ->
                    "Retailer preferred camera/OCR data over the catalog candidate."
                else ->
                    "No useful correction signal recorded."
            }
        )
    }
}
