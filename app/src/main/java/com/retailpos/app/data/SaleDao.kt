...
abstract class SaleDao {
    ...
    @Query("SELECT paymentMethod, total FROM sales WHERE storeId = :storeId AND createdAt >= :start AND createdAt < :end ORDER BY createdAt DESC")
    abstract suspend fun getPaymentRows(storeId: String, start: Long, end: Long): List<PaymentRow>

    suspend fun getNormalizedPaymentSummary(storeId: String, start: Long, end: Long): List<PaymentSummary> {
        val totals = linkedMapOf<String, Double>()
        val counts = linkedMapOf<String, Int>()
        for (row in getPaymentRows(storeId, start, end)) {
            val split = SplitPaymentRules.decode(row.paymentMethod)
            if (split.isEmpty()) {
                totals[row.paymentMethod] = (totals[row.paymentMethod] ?: 0.0) + row.total
                counts[row.paymentMethod] = (counts[row.paymentMethod] ?: 0) + 1
            } else {
                split.forEach { part ->
                    totals[part.method] = (totals[part.method] ?: 0.0) + part.amount
                    counts[part.method] = (counts[part.method] ?: 0) + 1
                }
            }
        }
        return totals.entries
            .sortedByDescending { it.value }
            .map { (method, total) -> PaymentSummary(method, counts[method] ?: 0, total) }
    }
    ...
    abstract suspend fun getTopProducts(storeId: String, start: Long, end: Long, limit: Int = 10): List<TopProductSales>
