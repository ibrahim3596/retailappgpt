...
fun SupplierPurchaseHistoryScreen(
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("SUPPLIERS & PURCHASES", fontWeight = FontWeight.Black) }, navigationIcon = { TextButton(onClick = onBack) { Text("BACK") } }) }) { padding ->
        LazyColumn(Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            item {
                Button(onClick = onNewPurchase, modifier = Modifier.fillMaxWidth()) { Text("NEW PURCHASE") }
            }
            ...
            items(suppliers, key = { it.id }) { supplier ->
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        ...
                        Text("Payable ₹${money(balance)}", color = if (balance > 0.0) MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.onSurfaceVariant, fontWeight = FontWeight.Bold)
            ...
            items(purchases, key = { it.id }) { purchase ->
                ...
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        ...
                        Text("Net ₹${money(purchase.netAmount)} • Paid ₹${money(purchase.paidAmount)} • Due ₹${money(purchase.outstandingAmount)}")
                    }
                }
            }
        }
            ...
            onSave = { amount, note ->
                scope.launch {
                    supplierLedgerDao.insert(
                        SupplierLedgerEntry(UUID.randomUUID().toString(), storeId, supplier.id, -amount, "PAYMENT", note.ifBlank { "Supplier payment" }, "SUPPLIER_PAYMENT", UUID.randomUUID().toString(), System.currentTimeMillis())
                    )
