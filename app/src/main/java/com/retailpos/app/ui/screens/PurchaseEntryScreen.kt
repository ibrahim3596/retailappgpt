            ...
            item {
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        OutlinedButton(onClick = { showSupplierDialog = true }, modifier = Modifier.fillMaxWidth()) { Text("ADD SUPPLIER") }
...
private fun SupplierPicker(selected: SupplierEntity?, suppliers: List<SupplierEntity>, onSelect: (SupplierEntity) -> Unit) {
    ...
    ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = !expanded }) {
        ...
        DropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }, modifier = Modifier.heightIn(max = 320.dp)) {
            suppliers.forEach { supplier -> DropdownMenuItem(text = { Text(supplier.name) }, onClick = { onSelect(supplier); expanded = false }) }
        }
    }
}
...
private fun PurchaseLineDialog(
    ...
    var expanded by remember { mutableStateOf(false) }
    var ordered by remember { mutableStateOf("1") }
    var free by remember { mutableStateOf("0") }
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Add purchase item") }, text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedTextField(discount, { discount = it }, singleLine = true, label = { Text("Scheme discount") })
        OutlinedTextField(batch, { batch = it }, singleLine = true, label = { Text("Batch number") })
        OutlinedTextField(expiry, { expiry = it }, singleLine = true, label = { Text("Expiry YYYY-MM-DD") })
        error?.let { Text(it, color = MaterialTheme.colorScheme.error) }
