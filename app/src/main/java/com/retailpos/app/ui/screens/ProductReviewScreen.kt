...
fun ProductReviewScreen(
    ...
    LaunchedEffect(scannedBarcode) { if (!scannedBarcode.isNullOrBlank()) { barcode = scannedBarcode; errorMessage = null } }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(if (isEdit) "EDIT PRODUCT" else "ADD PRODUCT", fontWeight = FontWeight.Black) }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).verticalScroll(rememberScrollState()).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                ...
                TextButton(onClick = onScanBarcode, modifier = Modifier.padding(top = 4.dp)) { Text("SCAN") }
            }
            Text("Supported product barcodes only. QR codes are not accepted as product identifiers.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            ...
            Text("Sale price cannot exceed MRP. Stock is managed separately after product creation.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            errorMessage?.let { Text(it, color = MaterialTheme.colorScheme.error) }
            Button(onClick = {
                val mrpValue = mrp.toDoubleOrNull() ?: -1.0; val saleValue = sellingPrice.toDoubleOrNull() ?: -1.0
                val purchaseValue = purchasePrice.toDoubleOrNull() ?: -1.0; val stockValue = stock.toDoubleOrNull() ?: -1.0; val thresholdValue = lowStockThreshold.toDoubleOrNull() ?: -1.0
