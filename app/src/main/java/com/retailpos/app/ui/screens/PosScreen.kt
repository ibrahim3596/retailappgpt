    ...
    Scaffold(
        ...
        Column(Modifier.fillMaxSize().padding(padding)) {
            LazyColumn(Modifier.weight(1f), contentPadding = PaddingValues(horizontal = 16.dp, vertical = 12.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                item {
                    OutlinedTextField(
                        value = query, onValueChange = { query = it; onSearchQueryChanged(it) }, modifier = Modifier.fillMaxWidth().height(58.dp), singleLine = true,
                        leadingIcon = { Icon(Icons.Default.Search, contentDescription = null) }, placeholder = { Text("Search name, barcode or SKU") }, trailingIcon = { IconButton(onClick = onOpenScanner) { Icon(Icons.Default.CameraAlt, contentDescription = "Scan") } }
                    )
                }
                if (recoveryIssues.isNotEmpty()) item {
                    Surface(color = MaterialTheme.colorScheme.errorContainer, shape = MaterialTheme.shapes.medium) {
                        Column(Modifier.fillMaxWidth().padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            Text("Bill needs attention", fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onErrorContainer)
                            recoveryIssues.forEach { Text("• ${ActiveCartRecovery.message(it)}", color = MaterialTheme.colorScheme.onErrorContainer) }
                            TextButton(onClick = onClearBill) { Text("Clear affected bill") }
                        }
                    }
                }
                if (!showingSearch) {
                ...
                if (showingSearch) {
                    ...
                    if (searchResults.isEmpty()) item {
                        Surface(color = MaterialTheme.colorScheme.surfaceVariant, shape = MaterialTheme.shapes.medium) {
                            Column(Modifier.fillMaxWidth().padding(18.dp), verticalArrangement = Arrangement.spacedBy(5.dp)) { Icon(Icons.Default.Search, contentDescription = null); Text("No products found", fontWeight = FontWeight.SemiBold); Text("Try another name, SKU or barcode.", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                        }
                    } else items(searchResults, key = { it.id }) { product ->
                        Surface(shape = MaterialTheme.shapes.medium, tonalElevation = 1.dp) {
                            Row(Modifier.fillMaxWidth().padding(13.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                                Column(Modifier.weight(1f)) { Text(product.name, fontWeight = FontWeight.SemiBold); if (product.brand.isNotBlank()) Text(product.brand, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant); Text("${product.stock.clean()} ${product.unit}  ·  ${money(product.sellingPrice)}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
                                IconButton(onClick = { onAddProduct(product) }, enabled = product.stock > 0) { Icon(Icons.Default.AddShoppingCart, contentDescription = "Add ${product.name}") }
                            }
                        }
                    }
                }
                        ...
                        Surface(shape = MaterialTheme.shapes.medium, tonalElevation = 1.dp) {
                            Column(Modifier.fillMaxWidth().padding(14.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                                Row(verticalAlignment = Alignment.CenterVertically) {
                                    Column(Modifier.weight(1f)) { Text(line.name, fontWeight = FontWeight.SemiBold); Text("${displayQuantity(line.quantity)} ${line.unit} × ${money(line.unitPrice)}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
                                    Text(money(line.lineTotal), style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                                ...
                                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(7.dp)) {
                                    ...
                                    OutlinedButton(onClick = { onSetCartQuantity(line, line.quantity - step) }, enabled = line.quantity - step > 0, modifier = Modifier.width(50.dp), contentPadding = PaddingValues(0.dp)) { Icon(Icons.Default.Remove, contentDescription = "Decrease") }
                                    OutlinedTextField(value = quantityText, onValueChange = { quantityText = it.filter { c -> c.isDigit() || c == '.' || c == ',' } }, singleLine = true, modifier = Modifier.weight(1f), label = { Text("Quantity") })
                                    Button(onClick = { onSetCartQuantity(line, line.quantity + step) }, modifier = Modifier.width(50.dp), contentPadding = PaddingValues(0.dp)) { Text("+") }
                                    TextButton(onClick = { quantityText.replace(',', '.').toDoubleOrNull()?.let { onSetCartQuantity(line, it) } ?: run { quantityText = displayQuantity(line.quantity) } }) { Text("Set") }
                                }
                            }
                        }
                    }
            ...
            Surface(tonalElevation = 4.dp, shadowElevation = 8.dp) {
                Column(Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 11.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                    Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                        Column(Modifier.weight(1f)) { Text("Total", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant); Text(money(total), style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold) }
                        Text("${itemCount.clean()} items", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(onClick = onCheckout, enabled = cart.isNotEmpty() && recoveryIssues.isEmpty(), modifier = Modifier.weight(1.8f).height(54.dp)) { Text("Charge ${money(total)}", fontWeight = FontWeight.Bold) }
                    }
                }
            }
        }
